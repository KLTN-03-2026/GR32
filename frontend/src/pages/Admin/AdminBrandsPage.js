...
const API = `${API_BASE}/api/admin/brands`;
...
const AdminBrandsPage = () => {
  ...
  const [meta, setMeta] = useState({ ma_thuong_hieu: "", ngay_tao: "", so_san_pham: 0 });
  ...
  const openAdd = () => {
    ...
    setEditId(null);
    setForm({ ...emptyForm });
    setMeta({ ma_thuong_hieu: "(Tự sinh)", ngay_tao: new Date().toISOString(), so_san_pham: 0 });
    setModal("add");
  };
  ...
  const openEdit = async (row) => {
    ...
    try {
      ...
      setForm((f) => ({ ...f, hinh_anh: res.data.hinh_anh || f.hinh_anh }));
    } catch {
      /* giữ từ bảng */
    }
  };
  ...
  const submitForm = async (e) => {
    ...
    setMsg("");
    const body = {
      ten_thuong_hieu: form.ten_thuong_hieu.trim(),
  ...
  const handleDelete = async (row) => {
    const ok = window.confirm("Bạn có chắc chắn muốn xóa thương hiệu khỏi hệ thống?");
    if (!ok) return;
    setMsg("");
    try {
  ...
  return (
    <div className="acp-page">
      <div className="acp-head">
        <div>
          <h2 className="acp-title">Quản lý thương hiệu</h2>
          <p className="acp-sub">Thêm, sửa, xóa thương hiệu (Admin / Nhân viên). Chỉ xóa được khi chưa gắn sản phẩm.</p>
        </div>
      ...
      <form className="acp-toolbar" onSubmit={applySearch}>
        <label className="acp-search-label">
          <span>Tìm kiếm (Mã TH, Tên, Slug):</span>
      ...
      {loading ? (
        ...
        <>
              ...
              <tbody>
                {items.map((row, i) => (
                  <tr key={row._id}>
                    ...
                    <td>
                      {row.hinh_anh ? (
                        <img className="abp-thumb" src={imgSrc(row.hinh_anh)} alt="" />
                      ) : (
                        <span className="acp-muted">—</span>
                      )}
                    </td>
                    <td className="acp-mono">{row.ma_thuong_hieu}</td>
          ...
          {items.length === 0 && <p className="acp-muted">Không có thương hiệu phù hợp.</p>}
          {totalPages > 1 && (
      ...
      {modal && (
        <div className="acp-modal-overlay" role="presentation" onClick={closeModal}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="abp-modal-title"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="acp-modal-head">
              <h3 id="abp-modal-title">{modal === "add" ? "Thêm thương hiệu" : "Sửa thương hiệu"}</h3>
            ...
            <form onSubmit={submitForm} className="acp-modal-body">
              <div className="acp-grid2">
                <label className="acp-field">
                  ...
                  <input type="text" readOnly value={meta.ma_thuong_hieu} className="acp-input acp-input--ro" />
                </label>
                <label className="acp-field">
                  <span>Ngày</span>
                ...
                <label className="acp-field">
                  ...
                  <input
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, ten_thuong_hieu: e.target.value }))}
                    placeholder="VD: ZARA"
                  />
                </label>
                <label className="acp-field">
              ...
              <label className="acp-field acp-field--full">
                <span>Ảnh (URL tùy chọn, ví dụ /uploads/...)</span>
                <input
                  value={form.hinh_anh}
              ...
              {form.hinh_anh && (
                <div className="abp-preview-wrap">
                  <img className="abp-preview" src={imgSrc(form.hinh_anh)} alt="" />
                </div>
              )}
              <label className="acp-field acp-field--full">
              ...
              <div className="acp-modal-actions">
                ...
                <button type="submit" className="acp-btn acp-btn--primary" disabled={saving}>
                  {saving ? "Đang lưu..." : modal === "add" ? "Thêm thương hiệu" : "Lưu thay đổi"}
                </button>
              </div>
            </form>
          </div>
