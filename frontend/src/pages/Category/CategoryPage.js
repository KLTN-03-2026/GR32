...
const API_FACETS = `${API_BASE}/api/products/filter-facets`;
...
const CATEGORY_MAP = {
  ...
  "quan-jean-nam": {
    name: "Quần jean nam",
    parent: "Thời trang Nam",
    dbValue: "Quần jean nam",
  },
  "quan-short-nam": {
    name: "Quần short nam",
    parent: "Thời trang Nam",
    dbValue: "Quần short nam",
  },
  "ao-thun-nu": {
    name: "Áo thun nữ",
...
const CategoryPage = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (selectedBrands.length)
        url += `&thuong_hieu=${selectedBrands.join(",")}`;
      if (priceRange.min > 0) url += `&gia_min=${priceRange.min}`;
      if (priceRange.max > 0) url += `&gia_max=${priceRange.max}`;
  ...
  useEffect(() => {
    const fetchFacets = async () => {
      try {
        const res = await axios.get(API_FACETS);
        setFacets(res.data);
        setPriceRange({
          min: res.data.gia_min || 0,
          max: res.data.gia_max || 0,
        });
      } catch (err) {
  ...
  useEffect(() => {
    setPage(1);
  }, [sortBy, selectedSizes, selectedColors, selectedBrands, priceRange]);
    ...
    <div className="category-page">
      ...
      <div className="category-layout">
        ...
        <aside className="cat-sidebar">
          ...
          <div className="cat-filter-group">
            <p className="cat-filter-title">Thương hiệu</p>
          ...
          <div className="cat-filter-group">
            <p className="cat-filter-title">Khoảng giá</p>
            <div className="cat-price-range">
              <input
                type="number"
                placeholder="Từ"
